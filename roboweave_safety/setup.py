from setuptools import find_packages, setup

package_name = "roboweave_safety"

setup(
    name=package_name,
    version="0.1.0",
    packages=find_packages(exclude=["tests"]),
    data_files=[
        ("share/ament_index/resource_index/packages", ["resource/" + package_name]),
        ("share/" + package_name, ["package.xml"]),
        ("share/" + package_name + "/config", [
            "config/safety_params.yaml",
            "config/workspace_limits.yaml",
        ]),
        ("share/" + package_name + "/launch", ["launch/safety.launch.py"]),
    ],
    install_requires=["setuptools", "roboweave-interfaces"],
    zip_safe=True,
    maintainer="RoboWeave",
    maintainer_email="roboweave@example.com",
    description="Independent safety supervisor for RoboWeave",
    license="MIT",
    entry_points={
        "console_scripts": [
            "safety_supervisor = roboweave_safety.safety_supervisor_node:main",
        ],
    },
)
